...
### 3-2. GitHub KB에서 프로젝트 문서를 읽어요 — KB fetch 안정성 개선 (Hotfix)

- spec-harness-kit 폴더가 git 저장소가 아니어도 정상입니다. git repo 여부와 GitHub KB 읽기는 무관합니다.
- `workspace/reference/existing/` 폴더에 XLSX만 있어도 정상입니다. decisions/meetings는 이 폴더가 아닌 GitHub KB에서 읽습니다.
- 기존 프로젝트에서 decisions/meetings fetch에 실패하면 조용히 넘어가지 않고 사용자에게 확인을 요청합니다.
