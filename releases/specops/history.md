...
---
## v1.3.6 (2026-09-10) — hotfix2: 2026-09-22

ZIP: `spec-harness-kit-v1.3.6-20260910.zip` · 파일 수: 59개

**hotfix2 (2026-09-22)**: `_updated` 정책 실행 순서 보완 — project-flow.md에서 `_updated` 갱신을 완료 출력 앞으로 이동, 갱신 실패 시 부분 실패 출력. check 98/99 강화(4요소), check 100/101 신규.

**hotfix1 (2026-09-22)**: `project-context.md _updated` 자동 갱신 정책 최초 적용 — CLAUDE.md·SKILL.md·spec-flow.md·project-flow.md 정책 추가, check 98/99 신규, sheetric project-context 날짜 보정.
...
- write는 검증을 통과한 모든 change를 단일 `spreadsheets.batchUpdate` 호출로 원자적으로 처리합니다. write 직후 `post_write_verification`을 수행하되, `update`/`fill_blank_row`는 실제 값을 재조회해 대조하고, `insert_row`/`delete_row`는 값을 재조회하지 않고 batchUpdate 성공 여부로만 판단합니다(정확한 사후 행 위치 재조회는 v1 범위 밖).
...
- `workspace/qa/`도 `workspace/.reports/`·`prototype/`·`project/`와 동일하게 `.gitkeep` 외 파일은 ZIP에서 제외됩니다.
